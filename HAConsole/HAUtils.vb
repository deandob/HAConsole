...
#Region "Misc"
    ...
    Public Sub WriteConsole(DisplayDebug As Boolean, ConsoleStr As String, <Runtime.CompilerServices.CallerMemberName> Optional CallerName As String = "")
        If DisplayDebug = True Or HAConsole.DebugMode = True Then
            Console.WriteLine(Date.Now.ToString("HH:mm:ss.fff") + " [" + CallerName + "]" + vbTab + ConsoleStr) ' Append a Date/Time stamp
            If HAServices.HomeServices.ClientLogs <> "" Then                ' Client viewing logs
                Dim myMessage As New Commons.Structures.HAMessageStruc With {.Category = "SYSTEM", .ClassName = "CONSOLE", .Instance = "LOGS", .Scope = "TEXT"}
                HomeNet.SendClient(HAServices.HomeServices.ClientLogs, Commons.HAConst.MessFunc.RESPONSE, myMessage, Date.Now.ToString("HH:mm:ss.fff") + " [" + CallerName + "]" + vbTab + ConsoleStr)
            End If
        End If
    End Sub
